...
//    'ignoreUnimportantViews' => '',//调用uiautomator的函数setCompressedLayoutHierarchy(),默认为false
    ...
    $test->byClassName('android.widget.CheckBox')->click();
...
//    'ignoreUnimportantViews' => '',//调用uiautomator的函数setCompressedLayoutHierarchy(),默认为false
    ...
    $test->startActivity(array("appPackage" => "tv.danmaku.bili",
        "appActivity" => ".MainActivityV2"));//打开app应用

//    $test->waitUntil(10);
    $test->byId('tv.danmaku.bili:id/expand_search')->click();

    $test->byId('tv.danmaku.bili:id/search_src_text')->value('text');

    $test->byId('tv.danmaku.bili:id/action')->click();

    $test->byId('tv.danmaku.bili:id/expand')->click();

    $test->byId('tv.danmaku.bili:id/clear')->click();

    $test->byId('android:id/button1')->click();

    $test->close();

//    $test->byXPath("//*[@content-desc='频道']")->click();

//    $test->byId('tv.danmaku.bili:id/update_cancel')->click();
}
